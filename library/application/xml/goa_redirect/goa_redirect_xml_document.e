...
feature -- Adding Elements
	...
	start_redirect_element (new_page_title: STRING; new_delay: STRING; new_url: STRING; new_style_sheet: STRING; )
			...
			-- Use end_current_element when done adding sub-elements to this element
		require
			ok_to_add_redirect: not root_element_added
			...
			is_valid_value_new_delay: new_delay /= Void implies is_valid_attribute_value (delay_attribute_code, new_delay)
			valid_new_delay: new_delay /= Void
		...
		local
			name_index, value_index: INTEGER
		do
			writer.start_ns_tag ("", element_tag_for_code (redirect_element_code))
			root_element_added := True
			writer.set_a_name_space ("goa_redirect", "http://www.sourceforge.net/projects/goanna/goa_redirect")
			...
			if new_delay /= Void then
				writer.set_attribute (attribute_name_for_code (delay_attribute_code), new_delay)
			end
			if new_url /= Void then
				writer.set_attribute (attribute_name_for_code (url_attribute_code), new_url)
			end
			...
			current_element_contents.force (redirect_element_code, current_element_contents.upper + 1)
			element_stack.put (redirect_element_code)
			contents_stack.put (<<>>)
		ensure
			root_element_added: root_element_added
			open_element_count_updated: element_stack.count = old element_stack.count + 1
			current_element_updated: current_element_code = redirect_element_code
	...
	is_valid_element_content_fragment (the_element_code: INTEGER; the_fragment: ARRAY [INTEGER]): BOOLEAN
		...
		do
			inspect the_element_code
				...
				when redirect_element_code then
					Result := redirect_content_validity.is_valid_content_fragment (the_fragment)
			else
				Result := False
			end
		end
	...
	is_valid_element_content (the_element_code: INTEGER; the_content: ARRAY [INTEGER]): BOOLEAN
		...
		do
			inspect the_element_code
				...
				when redirect_element_code then
					Result := redirect_content_validity.is_valid_content (the_content)
			else
				Result := False
			end
		end
